...
1. **工具调用**：城市信息、预算拆分、安全提醒、行程骨架生成。
2. **会话记忆**：按 `sessionId` 保存用户偏好和最近对话，下一轮自动复用城市、预算、天数、偏好和同行人群。
3. **轻量 RAG**：基于内置旅游知识库进行关键词检索，补充城市、预算、安全和人群建议。
4. **评测脚本**：检查回复关键词覆盖、工具调用数量、RAG 命中和记忆写入。
...
> AI 智能旅游规划 H5：基于 Vue3 + Vant + Node/Express + LangChain 实现移动端 AI 行程规划系统，支持多维旅行偏好输入、SSE 流式问答、结构化行程生成、本地历史管理；后端扩展 Travel Agent v1，包含工具调用、Session 记忆、轻量 RAG 和离线评测脚本，并设计本地兜底策略保障演示可用性。

## 开源注意事项

- 不要提交 `.env`、真实 API Key、用户记忆数据和评测输出数据。
- `node_modules/`、`dist/`、服务端日志和本地数据应保持忽略。
- 若上传 GitHub，建议先确认 `travel-server/.env.example` 中只保留示例配置。
